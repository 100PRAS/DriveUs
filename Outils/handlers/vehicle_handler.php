...
if ($action === 'get_vehicles') {
    $stmt = $pdo->prepare("SELECT id, model, plate, year, seats, fuel_type, spec_file, created_at FROM user_vehicles WHERE UserID = ? ORDER BY created_at DESC");
    $stmt->execute([$userId]);
...
if ($action === 'add_vehicle') {
    ...
    // Insérer le véhicule
    try {
        $stmt = $pdo->prepare("INSERT INTO user_vehicles (UserID, model, plate, year, seats, fuel_type, spec_file) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$userId, $model, $plate, $year, $seats, $fuelType, $specFile]);
...
if ($action === 'delete_vehicle') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT spec_file FROM user_vehicles WHERE id = ? AND UserID = ?");
